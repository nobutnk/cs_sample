Class GitClientUtils {
  ...
  public File getFileFromGitRepository(string repositoryName, string url)
  {
    ...
    try {
        ...
        GitVersionDescriptor getDes = new GitVersionDescriptor() {Version = "your-branch-name"};

        // フォルダ配下取得するときは、GetItemsAsyncで、scopePathをフォルダ単位で設定
        var items = await client.GetItemsAsync(repositoryId, scopePath:"/Solution1/ConsoleApplication1", versionDescriptor:gitDes, recursionLevel:VersionControlRecursionType.Full);
        ...
        var item = await client.GetItemAsync(repositoryId, path:"ファイルパス設定", versionDescriptor:gitDes);
        var blob = await client.GetBlobContentAsync(repositoryId, item.ObjectId, ...);
        // あとは省略。Streamからファイル化してください。

        // 別手法。これで１行で取得できそう。引数はもっとある。
        // var blobはTask<Stream>のこと
        var blob = await client.GetItemContentAsync(repositoryId, path:"ファイルパス設定", versionDescriptor:gitDes);
        // あとは省略。Streamからファイル化してください。
    }
    catch(Exception e)
    {
